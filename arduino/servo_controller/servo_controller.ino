#include <Servo.h>

// ── 設定 ──────────────────────────────────
#define SERVO_PIN       9
#define SERIAL_BAUD     115200
#define MAX_STEPS       48      // 64→48，省 SRAM 96 bytes
#define BUF_SIZE        48      // 64→48，夠用且省 SRAM

#define SPD_SLOW        20      // speed=1  每度延遲 ms
#define SPD_FAST        1       // speed=100 每度延遲 ms

// ── F() 巨集：字串放 Flash 不佔 SRAM ─────
// ── 回應字串：用短碼節省 Flash ────────────
// 所有 Serial.println 改用 F() 把字串存 Flash

// ── 資料結構 ──────────────────────────────
struct Step {
  uint16_t delay_ms;
  uint16_t duration_ms;
  uint8_t  angle;
  uint8_t  speed;
};

// ── 全域變數 ──────────────────────────────
Servo   servo;
Step    steps[MAX_STEPS];
uint8_t stepCount     = 0;
uint8_t curStep       = 0;
uint8_t expSteps      = 0;
int16_t curAngle      = 0;
bool    running       = false;
bool    looping       = false;
bool    recving       = false;
char    buf[BUF_SIZE];

// ── 輔助：手動解析數字，取代 sscanf ───────
// 省下 sscanf 約 1800 bytes Flash
static char* parseU16(char *p, uint16_t &out) {
  while (*p == ' ') p++;
  uint16_t v = 0;
  while (*p >= '0' && *p <= '9') v = v * 10 + (*p++ - '0');
  out = v;
  return p;
}
static char* parseU8(char *p, uint8_t &out) {
  while (*p == ' ') p++;
  uint16_t v = 0;
  while (*p >= '0' && *p <= '9') v = v * 10 + (*p++ - '0');
  out = (uint8_t)v;
  return p;
}

// ── Servo 平滑移動 ─────────────────────────
void moveServo(int16_t target, uint8_t spd) {
  spd = constrain(spd, 1, 100);
  int dpd = map(spd, 1, 100, SPD_SLOW, SPD_FAST);
  int16_t dir = (target > curAngle) ? 1 : -1;
  while (curAngle != target) {
    curAngle += dir;
    servo.write(curAngle);
    delay(dpd);
  }
}

// ── 執行單步 ──────────────────────────────
void execStep(Step &s) {
  if (s.delay_ms > 0) {
    uint16_t w = 0;
    while (w < s.delay_ms) {
      delay(10);
      w += 10;
      if (!running) return;
    }
  }
  moveServo(s.angle, s.speed);
  if (s.duration_ms > 0) delay(s.duration_ms);
  moveServo(0, s.speed);
}

// ── 回應輔助（F() 把字串存 Flash）─────────
inline void ok(const __FlashStringHelper *s)  { Serial.println(s); }
inline void err(const __FlashStringHelper *s) { Serial.println(s); }

// ── 指令解析 ──────────────────────────────
void processCommand(char *p) {

  // PING
  if (!strcmp(p, "PING"))   { ok(F("OK PONG"));    return; }

  // STATUS
  if (!strcmp(p, "STATUS")) {
    if (running) {
      Serial.print(F("OK RUNNING "));
      Serial.print(curStep + 1);
      Serial.print('/');
      Serial.println(stepCount);
    } else {
      ok(F("OK IDLE"));
    }
    return;
  }

  // STOP
  if (!strcmp(p, "STOP")) {
    running = recving = false;
    curStep = 0;
    moveServo(0, 80);
    ok(F("OK STOPPED"));
    return;
  }

  // END
  if (!strcmp(p, "END")) {
    if (!recving)               { err(F("ERR NO_BEGIN"));      return; }
    if (stepCount != expSteps)  { err(F("ERR STEP_MISMATCH")); recving = false; return; }
    recving = false;
    curStep = 0;
    running = true;
    ok(F("OK RUNNING"));
    return;
  }

  // LOOP 0/1
  if (!strncmp(p, "LOOP ", 5)) {
    looping = (p[5] == '1');
    Serial.print(F("OK LOOP "));
    Serial.println(looping ? '1' : '0');
    return;
  }

  // BEGIN n
  if (!strncmp(p, "BEGIN ", 6)) {
    if (running)                          { err(F("ERR BUSY"));     return; }
    expSteps = atoi(p + 6);
    if (!expSteps || expSteps > MAX_STEPS){ err(F("ERR OVERFLOW")); return; }
    stepCount = 0;
    recving   = true;
    ok(F("OK RECEIVING"));
    return;
  }

  // STEP d a sp dur
  if (!strncmp(p, "STEP ", 5) && recving) {
    if (stepCount >= MAX_STEPS) { err(F("ERR OVERFLOW")); return; }
    uint16_t d, dur;
    uint8_t  a, sp;
    char *q = p + 5;
    q = parseU16(q, d);
    q = parseU8 (q, a);
    q = parseU8 (q, sp);
    q = parseU16(q, dur);
    if (!*q && q == p + 5)      { err(F("ERR PARSE")); return; }
    steps[stepCount].delay_ms    = d;
    steps[stepCount].angle       = constrain(a,  0, 180);
    steps[stepCount].speed       = constrain(sp, 1, 100);
    steps[stepCount].duration_ms = dur;
    stepCount++;
    Serial.print(F("OK STEP "));
    Serial.println(stepCount);
    return;
  }

  err(F("ERR UNKNOWN"));
}

// ── setup ──────────────────────────────────
void setup() {
  Serial.begin(SERIAL_BAUD);
  servo.attach(SERVO_PIN);
  servo.write(0);
  delay(500);
  ok(F("OK READY"));
}

// ── loop ───────────────────────────────────
void loop() {
  if (Serial.available()) {
    int len = Serial.readBytesUntil('\n', buf, BUF_SIZE - 1);
    buf[len] = '\0';
    if (len > 0 && buf[len-1] == '\r') buf[--len] = '\0';
    if (len > 0) processCommand(buf);
  }

  if (running && stepCount > 0) {
    execStep(steps[curStep]);
    if (!running) return;
    curStep++;
    if (curStep >= stepCount) {
      if (looping) {
        curStep = 0;
        ok(F("OK LOOP_RESTART"));
      } else {
        running = false;
        curStep = 0;
        ok(F("OK DONE"));
      }
    } else {
      Serial.print(F("OK RUNNING "));
      Serial.print(curStep + 1);
      Serial.print('/');
      Serial.println(stepCount);
    }
  }
}
